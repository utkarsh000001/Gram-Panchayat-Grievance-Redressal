...
# Admin credentials should be configured in Streamlit secrets. The fallback
# values keep the prototype runnable locally, but must be changed before use.
...
def get_admin_credentials():
    try:
        admin_id = st.secrets.get("ADMIN_ID", os.getenv("ADMIN_ID", DEFAULT_ADMIN_ID))
    ...
    return str(admin_id), str(admin_password)


def admin_login():
    """Render the Admin login and return True only after valid credentials."""
    if st.session_state.get("admin_authenticated", False):
        return True

    st.subheader("🔐 Panchayat Admin Login")
    st.info("The Citizen Portal is public. Admin credentials are required to open the grievance register.")
    with st.form("admin_login_form", clear_on_submit=False):
        entered_id = st.text_input("Admin ID", autocomplete="username")
    ...
    if submitted:
        ...
        valid_id = hmac.compare_digest(entered_id.strip(), expected_id)
        valid_password = hmac.compare_digest(entered_password, expected_password)
...
def get_conn():
    ...
    """)
    # Lightweight migrations for older prototype databases.
    for col, coltype in (("photo", "BLOB"), ("photo_ref", "TEXT"), ("resolved_at", "TEXT")):
        try:
...
def store_photo(photo_bytes):
    """Use S3 when configured; otherwise keep the prototype's local BLOB fallback."""
    if not photo_bytes:
        return None, None
    ...
    try:
        import boto3
        key = f"grievances/{datetime.datetime.now():%Y/%m}/{uuid.uuid4().hex}.jpg"
        boto3.client("s3", region_name=region).put_object(
            Bucket=bucket, Key=key, Body=photo_bytes, ContentType="image/jpeg"
        )
        return key, None
    except Exception as exc:
        st.warning(f"S3 upload unavailable; keeping the photo in local demo storage. ({exc})")
        return None, photo_bytes


def photo_uri(photo_ref, photo_bytes):
    if photo_ref:
        try:
            import boto3
            bucket = os.getenv("S3_BUCKET")
            ...
            if bucket:
                return boto3.client("s3", region_name=region).generate_presigned_url(
                    "get_object", Params={"Bucket": bucket, "Key": photo_ref}, ExpiresIn=3600
                )
        except Exception:
            pass
    if photo_bytes:
...
def add_grievance(category, description, location, phone, language, photo_bytes=None):
    ...
    photo_ref, local_photo = store_photo(photo_bytes)
    conn.execute(
        "INSERT INTO grievances (ticket_id, category, description, location, "
        "phone, language, status, created_at, photo, photo_ref) VALUES (?,?,?,?,?,?,?,?,?,?)",
        (ticket_id, category, description, location, phone, language,
         "Received", created_at, local_photo, photo_ref),
    )
    conn.commit()
    conn.close()
...
def get_all_df() -> pd.DataFrame:
    ...
    df["photo_uri"] = df.apply(
        lambda row: photo_uri(row.get("photo_ref"), row.get("photo")), axis=1
    )
...
def find_grievance(query: str):
    ...
    cols = ["id", "ticket_id", "category", "description", "location",
            "phone", "language", "status", "created_at", "photo", "photo_ref", "resolved_at"]
...
def render_live_tracker():
    if not st.session_state.get("tracking_query"):
        return
    st.divider()
    st.subheader("Live complaint tracker")
    st.caption("Refresh to see the latest Admin status update.")
    if st.button("↻ Refresh status", key="refresh_live_status"):
        st.rerun()
    live_match = find_grievance(st.session_state.tracking_query)
    if live_match:
        live_idx = STATUSES.index(live_match["status"])
        st.progress(
            (live_idx + 1) / len(STATUSES),
            text=f"{live_match['status']} · {live_match['ticket_id']}",
        )
        st.markdown(" → ".join(
            f"**{status}**" if i <= live_idx else status
...
st.caption("Public Citizen Portal · Protected Panchayat Admin Register — शिकायत दर्ज करें · निवारण पाएँ")
...
with tab_citizen:
    ...
    with col_info:
        ...
        if st.session_state.get("last_slip"):
            st.download_button(
                "⬇️ Download complaint slip",
                data=st.session_state.last_slip,
                file_name=f"{st.session_state.last_ticket_id}.txt",
                mime="text/plain",
    ...
    with col_chat:
        ...
        if step == "choose_language":
            ...
            st.caption("You can type your complaint or record it by voice.")
                ...
                if st.button("Transcribe recording", use_container_width=True):
                    try:
                        ...
                        transcript = recognizer.recognize_google(
                            audio_data, language="hi-IN" if st.session_state.data.get("language") == "हिन्दी" else "en-IN"
                        )
                        st.session_state.voice_transcript = transcript
                        st.success(f"Transcript: {transcript}")
                    except ImportError:
                        st.error("Speech-to-text is not installed. Add SpeechRecognition to requirements.txt.")
                    except Exception as exc:
        ...
        if step in ("describe", "location", "phone", "track_input"):
            ...
            text = st.chat_input(t(placeholder_key), key=f"chat_{step}")
            if step == "describe" and not text:
                text = st.session_state.pop("voice_transcript", "")
            if text:
                user(text)
                ...
                if step == "describe":
                    ...
                    if match is None:
                        ...
                        st.session_state.tracking_query = text
                    st.session_state.step = "done"
...
st.divider()
st.caption("Working prototype for PS-24 · Citizen Portal is public; Panchayat Admin requires credentials · "
